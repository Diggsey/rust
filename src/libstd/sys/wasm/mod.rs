...
unsafe fn syscall<T>(index: SysCallIndex, data: &mut T) -> bool {
    __rust_wasm_syscall(index as usize, data as *mut T as *mut Void)
}
...
#[no_mangle]
#[inline(never)]
unsafe fn __rust_wasm_syscall(index: usize, data: *mut Void) -> bool {
    extern {
        fn rust_wasm_syscall(index: usize, data: *mut Void) -> usize;
    }
    rust_wasm_syscall(index, data) != 0
}
...
#[no_mangle]
#[inline(never)]
unsafe fn __rust_wasm_syscall(_index: usize, _data: *mut Void) -> bool {
    // Prevent inlining since #[inline(never)] in not enough on the wasm32-unknown-unknown target.
    asm!("");
    false
}
