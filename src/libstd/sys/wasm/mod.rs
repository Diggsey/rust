...
impl ReadSysCall {
    pub fn perform(fd: usize, buffer: &mut [u8]) -> usize {
        ...
        unsafe { syscall(SysCallIndex::Read, &mut call_record); }
        call_record.result
    }
}
...
impl ArgsSysCall {
    pub fn perform() -> Vec<OsString> {
        receive_buffer(1024, |buffer| -> Result<usize, !> {
            ...
            unsafe { syscall(SysCallIndex::Args, &mut call_record); }
...
impl GetEnvSysCall {
    pub fn perform(key: &OsStr) -> Option<OsString> {
        ...
        receive_buffer(64, |buffer| {
            ...
            unsafe { syscall(SysCallIndex::GetEnv, &mut call_record); }
...
impl TimeSysCall {
    pub fn perform(clock: TimeClock) -> Duration {
        ...
        unsafe { syscall(SysCallIndex::Time, &mut call_record); }
        if call_record.secs_hi != 0 || call_record.secs_lo != 0 || call_record.nanos != 0 {
            Duration::new(
...
extern {
    fn rust_wasm_syscall(index: usize, data: *mut Void);
}
unsafe fn syscall<T>(index: SysCallIndex, data: &mut T) {
    rust_wasm_syscall(index as usize, data as *mut T as *mut Void);
}
